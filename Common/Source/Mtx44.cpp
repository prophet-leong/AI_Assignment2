...
void Mtx44::SetToLookAt(Vector3 eye, Vector3 center, Vector3 up)
{
	Vector3 f((float)(center.x - eye.x), (float)(center.y - eye.y), (float)(center.z - eye.z));
	f.Normalize();
	up.Normalize();
	Vector3 s = f.Cross(up);
	Vector3 u = s.Cross(f);
	...
	Mtx44 mat(s.x, u.x, -f.x, 0,
		...
		0, 0, 0, 1);
	Mtx44 tran;
	tran.SetToTranslation(-(float)eye.x, -(float)eye.y, -(float)eye.z);
	*this = mat * tran;
}
